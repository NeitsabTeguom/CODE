...
typedef struct _Demo_Counter Demo_Counter;
...
Demo_Counter* Demo_Counter_new();
Demo_Program* Demo_Program_new();
i64 Demo_Counter_Increment(Demo_Counter* self);
i64 Demo_Program_Main(code_string* args);
typedef struct LamEnv_0 {
    Demo_Counter* _app;
} LamEnv_0;
static void* lam_0_fn(void* __envRaw);
struct _Demo_Counter {
    i64 N;
};
...
Demo_Counter* Demo_Counter_new() {
    Demo_Counter* self = (Demo_Counter*) GC_MALLOC(sizeof(Demo_Counter));
    #line 5 "/tmp/test-c.am"
    self->N = 0;
    return self;
}

i64 Demo_Counter_Increment(Demo_Counter* self) {
    #line 8 "/tmp/test-c.am"
    Demo_Counter* app = self;
    #line 9 "/tmp/test-c.am"
    LamEnv_0* __env_0 = (LamEnv_0*) code_alloc(sizeof(LamEnv_0));
    __env_0->_app = app;
    AmalgameClosure* cb = AmalgameClosure_new((void*)lam_0_fn, __env_0);
    #line 13 "/tmp/test-c.am"
    (i64)(intptr_t)AmalgameClosure_call1(cb, (void*)(intptr_t)(0));
    #line 14 "/tmp/test-c.am"
    return app->N;
}
...
i64 Demo_Program_Main(code_string* args) {
    #line 20 "/tmp/test-c.am"
    Demo_Counter* c = Demo_Counter_new();
    #line 21 "/tmp/test-c.am"
    i64 r = Demo_Counter_Increment(c);
    #line 22 "/tmp/test-c.am"
    Console_WriteLine(code_string_concat("count=", String_FromInt(r)));
    #line 23 "/tmp/test-c.am"
    return 0;
}

static void* lam_0_fn(void* __envRaw) {
    LamEnv_0* __env = (LamEnv_0*)__envRaw;
    Demo_Counter* app = __env->_app;
    #line 10 "/tmp/test-c.am"
    app->N = (app->N + 1);
    #line 11 "/tmp/test-c.am"
