...
| MI4 | gRPC server (HTTP/2 + protobuf) | grpc-gen + serve | CORE | 🟡 partial | Two layers shipped (7/7 each, local audit 2026-06-05): `amalgame-formats-protobuf` v0.1.0 (proto3 wire codec, binary-safe) + `amalgame-net-grpc` v0.1.0 (framing + status + path + unary dispatch, transport-agnostic). **Blocking gap:** net-http nghttp2 lacks HTTP/2 trailers (`grpc-status`) + binary H2 body → not servable end-to-end yet. Next: extend net-http H2, then grpcurl-tested unary server + `.proto` codegen |
