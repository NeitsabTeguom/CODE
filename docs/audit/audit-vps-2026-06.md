...
| MD10 | OAuth2 PKCE + OIDC id_token | PKCE challenge + id_token validated | SHIPPED | ✅ PASS | **PKCE (RFC 7636 S256)** in `amalgame-web` v0.34.0 (`WithPkce()`) — challenge matches the RFC 7636 Appendix B vector. **OIDC id_token** in v0.35.0 (`WithOidc(issuer)`): HandleCallback verifies the id_token — **RS256 sig** vs the IdP's JWKS (pinned `jwks_uri` or `/.well-known/openid-configuration` discovery; key built **verify-only** via crypto v0.6.0 `JwsKey.FromJwkRsa` + `EVP_PKEY_fromdata`), **alg hard-pinned to RS256** (`none`/`HS*` rejected before any sig work → no algorithm-confusion), and `iss` / `aud` (string or array) / `exp` (+60s skew) / one-time `nonce` (replay) all enforced; fails **closed**. Local audit 2026-06-05: self-signed RS256 round-trip verifies, and 5 negative tests all reject — nonce mismatch, audience mismatch, issuer mismatch, tampered payload (sig invalid), HS256 alg-confusion. `VerifyIdTokenJwks` is a network-free seam. Wire to a real Google login on the VPS |
| MD11 | WebAuthn / passkeys | Passwordless registration + assertion | SHIPPED | ✅ PASS | `amalgame-webauthn` v0.1.0 (`WebAuthn` RP, on crypto v0.7.0). Local audit 2026-06-05 (8 tests): a **real ES256-signed assertion** verifies end-to-end — clientDataJSON (type/challenge/origin) → attestationObject CBOR decode → COSE EC2 key extraction → assertion signature over authData‖SHA-256(clientData) via `JwsKey.FromCoseEc2`+`VerifyDer`; and every guard rejects: **wrong origin**, **challenge mismatch**, **tampered signature**, **non-increasing counter (cloned authenticator)**, **rpIdHash mismatch**. Scope ES256 + attestation `none` (cert-chain attestation deferred — documented). Wire to a real passkey enroll/login on the VPS |
...
| MD13 | Typed input validation | Malformed / oversized / injection inputs rejected before handler | SHIPPED | ✅ PASS | `amalgame-web` v0.36.0 `Validator`/`FieldRule`/`ValidationResult` (`validation.am`): Required/Optional, strict Int/Float/Bool/Email, Range, Min/MaxLen, OneOf allow-lists, Alnum/Alpha/Numeric, AllowMultiline → `Check(map)` returns typed accessors + `ErrorJson()` 422. Local audit 2026-06-05 (11 tests): **CRLF in a single-line field rejected** (header/log-injection lever), **8 KiB default length cap** even with no MaxLen (anti-DoS), multiline allows LF but still blocks a BEL/NUL control byte, strict int (`"12abc"`→reject) + range, email + enum allow-list, alnum charset. Pure (never executes). Wire to the contact-form / API inputs on VPS |
| MD14 | Schema migrations (data integrity) | A failing migration leaves no half-applied schema; rollbacks safe | SHIPPED | ✅ PASS | `amalgame-database-migrate` v0.1.0 (`Migrator` over `amalgame-database-sqlite`). Local audit 2026-06-05 (9 tests): migrations apply in **ascending order regardless of registration**; **a failing migration is rolled back and the run STOPS** — verified the half-created table is absent and later migrations never ran (no schema drift); `Up` is **idempotent** (re-run applies nothing); `Down` **refuses an irreversible** (no-down) migration; **duplicate versions rejected**; `DownTo` unwinds to a target; tracking-table name is identifier-validated and applied rows use bound params. Use for the VPS app DBs instead of ad-hoc `Exec()` DDL |
...
| MI4 | gRPC server (HTTP/2 + protobuf) | grpc-gen + serve | SERVABLE (unary) | 🟢 PASS (e2e) | protobuf v0.1.0 + net-grpc **v0.2.0** (`GrpcServer.ServeH2c`) + net-http v0.22.0 (H2 trailers + binary body). **End-to-end proven 2026-06-05**: a compiled Amalgame gRPC server answers a real nghttp2 client over TCP — `:status 200` + `application/grpc` + `grpc-status 0` TRAILER + framed echo byte-exact, clean shutdown. **Full `.proto` → typed server codegen** shipped (formats-protobuf v0.2.1 `tools/proto-gen.js`: messages → AM classes + `service{}` → typed `<Name>Service.RegisterOn(GrpcServer)`; round-trip 4/4 + typed-dispatch 3/3). **Unary client** shipped (net-http v0.23.0 `H2Client` + net-grpc v0.3.0 `GrpcClient`) — **100%-Amalgame e2e proven**: AM `GrpcClient` ↔ AM `ServeH2c` server over TCP (echo incl. NUL + grpc-status 0). **Server streaming** shipped (net-grpc v0.4.0). **grpcurl interop PASS** (v0.4.1): the reference gRPC client calls a typed Greeter server → `{"message":"Hello, Ada"}` (in-suite + CI) — real gRPC-standard interop proven. **Typed client stubs** (proto-gen v0.2.2): a fully typed `.proto → client + server` round-trip tested (GreeterClient.SayHello ↔ GreeterService). **gRPC over TLS** (net-grpc v0.5.0 ServeHttps) validated by grpcurl-over-TLS. **gRPC client over TLS** (net-http v0.24.0 H2Client.ConnectTls + net-grpc v0.6.0 GrpcClient.DialTls) — typed client↔TLS-server e2e (unary+streaming). **All 4 method types** (net-grpc v0.7.0: +client-streaming +bidi, collect model). Remaining: true incremental flush, reflection |
