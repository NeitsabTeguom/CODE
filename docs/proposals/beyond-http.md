...
**Status:** in-progress. Verified against the repos **2026-06-22**.
...
> deliverability + security-audit gating). Phases 1-4 shipped:
> `amalgame-formats-mime` v0.1.0 (RFC 5322/MIME reader) +
> `amalgame-net-mail-store` v0.1.0 (Maildir + SQLite index) +
> `amalgame-net-smtp-server` v0.1.0 (receiving ESMTP: EHLO/STARTTLS/AUTH/
> DATA → store) + `amalgame-net-imap` v0.1.0 (IMAP4rev1: LOGIN/SELECT/
> FETCH/STORE/SEARCH/EXPUNGE) — both loopback smoke-tested with real
> clients. SMTP-in + IMAP-out work end to end. Still roadmap: net-pop3,
> DKIM/SPF, the security-audit gate before any :25 exposure.
...
> `HttpRequest -> HttpResponse`, mountable on a Mosaic host
> (`MosaicServer.AddHandler`) or a `WebApp` catch-all route, and
> unit-testable without a socket. **Class 1** (always on): OPTIONS,
> PROPFIND (Depth 0/1, 207 multistatus XML), GET, HEAD, PUT, DELETE,
> MKCOL, COPY, MOVE (Destination + Overwrite). **Class 2** (opt-in
> `.WithLocks()`): exclusive write LOCK / UNLOCK with If-header / 423
> enforcement. Security: operator-fixed root, `..` traversal rejected
> (403) before disk, `.ReadOnly()` mode, locks fail closed. Binary-safe:
> GET via net-http File mode, **streaming PUT to disk**, COPY streams via
> libc. Real `displayname` (decoded), `<D:href>` percent-encoded per
> RFC 3986, live `<oc:size>` directory sizes (xattr cache, ownCloud
> client compatible).
>
> **`WebDavNas`** — multi-tenant layer over `WebDav`: per-user `/home/<user>`
> + a shared `/shared`, with `DispatchAs(req, user)` routing a
> pre-authenticated principal (pairs with `amalgame-auth`). Synthetic
> listings (`/all`, group roots, NAS root) carry `<oc:size>`. **This is
